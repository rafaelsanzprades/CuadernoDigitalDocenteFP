    ...
    <div className="w-full flex flex-col z-40 sticky top-0 bg-background/95 backdrop-blur-xl border-b border-[var(--glass-border)] pb-2 shadow-md">
      ...
      <div className="w-full px-6 py-2 bg-foreground/[0.015] border-b border-[var(--glass-border)] grid grid-cols-1 lg:grid-cols-3 items-center gap-2 sm:gap-3">
        ...
        <div className="flex justify-start items-center gap-3 min-w-0">
          ...
          {mounted && (
            <div className="hidden sm:flex h-10 bg-foreground/5 p-1 rounded-xl border border-[var(--glass-border)] shadow-sm shrink-0">
              {/* Botón DEMO */}
              <button
                onClick={async () => {
                ...
                className={`flex flex-col items-center justify-center px-3 h-full rounded-lg transition-all ${
                  dataSource === 'demo'
          ...
          {dataSource === 'local' && !workspaceHandle ? (
            <button
              ...
              className={`w-full max-w-[200px] h-10 bg-foreground/5 border rounded-lg px-3 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-success/50 cursor-pointer text-left hover:bg-foreground/10 transition-colors shrink-0`}
              style={{ color: 'var(--success)', borderColor: 'var(--success)' }}
            >
              Archivos
            </button>
          ) : (
            <div className="relative w-[180px] sm:w-[200px] shrink-0 h-10">
              <select 
                className={`w-full h-full bg-foreground/5 border rounded-lg pl-3 pr-8 text-sm font-semibold focus:outline-none focus:ring-2 cursor-pointer appearance-none text-left transition-colors`}
                style={{ 
